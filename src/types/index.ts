...
export interface PayrollRun {
    ...
    createdAt: string;
    autoGenerated?: boolean; // created by the "Auto-run payroll" setting, not a manual button click
}
...
export interface BusinessSettings {
    ...
    // When true, whichever period getPayrollReminderStatus flags as due gets
    // run automatically (same flat-rate ManualPayrollProvider math as the
    // "Run Payroll" button) instead of only showing a reminder banner. Still
    // record-keeping only -- no real money moves, no real tax withholding.
    payrollAutoRunEnabled?: boolean;
    payrollAutoRunDeductionRate?: number; // flat % used for automatic runs; treated as 0 when unset
