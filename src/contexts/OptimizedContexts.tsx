...
export function FinanceProvider({ children }: { children: ReactNode }) {
  ...
  // Shared by the manual "Run Payroll" button (runPayroll below) and the
  // automatic-run effect further down -- both just compute a PayrollRun +
  // its linked Salaries expense from a set of items; the only difference
  // is who decided which period to run and whether autoGenerated is set.
  const commitPayrollRun = (period: string, items: PayrollItem[], existingTransactionId: string | undefined, autoGenerated: boolean) => {
    ...
    const run: PayrollRun = {
      ...
      totalNet, status: 'paid', transactionId: txId, createdAt: now, autoGenerated,
    };
    setPayrollRuns((prev) => [...prev, run]);
  };

  // Automatic payroll: when enabled, whichever period the reminder banner
  // would otherwise just flag as due/overdue gets run for real (recorded,
  // same as the manual button) instead of waiting for someone to tap
  // "Run Payroll". Still the flat-rate ManualPayrollProvider calculation --
  // no real money moves, no real tax withholding -- this only removes the
  // manual click for businesses that don't want to remember it monthly.
  useEffect(() => {
    if (!hydrated || isDemoMode || isStaffRole) return;
    if (!settingsForFinance?.settings?.payrollAutoRunEnabled) return;
    const auto = computeAutoPayrollRun(staff, payrollRuns, settingsForFinance?.settings?.payrollAutoRunDeductionRate ?? 0);
    if (!auto) return;
    commitPayrollRun(auto.period, auto.items, undefined, true);
  }, [hydrated, isDemoMode, isStaffRole, staff, payrollRuns, settingsForFinance?.settings?.payrollAutoRunEnabled, settingsForFinance?.settings?.payrollAutoRunDeductionRate]);
  ...
  const value: FinanceContextValue = useMemo(
    () => ({
      ...
      // A payroll payment that already exists as an imported bank transaction
      // (tagged "Payroll" -- see computeUnlinkedPayrollTransactions) is linked
      // to this run instead of getting a second, duplicate expense: the run's
      // own items/totals are computed from real Staff records either way,
      // only where the matching cash movement is recorded differs.
      runPayroll: (period, items, _deductionRate, existingTransactionId) => commitPayrollRun(period, items, existingTransactionId, false),
      deletePayrollRun: (id) => {
