...
// password first, then a typed 6-digit email code (not a full "forgot PIN"
// reset, and not a clicked link either -- a link only works once and can
// land in the wrong browser if mail happens to open somewhere else),
// unlike the per-device secret the PIN normally unlocks. See
// supabase/functions/password-login for the full design and why this is
// intentionally a separate credential from that device secret.
...
// itself: the caller follows up with a typed 6-digit email code as a
// second factor, since this is reachable with no session at all.
