        ...
        <SafeAreaView style={styles.safe}>
            ...
            <View style={styles.exportBar}>
                <TouchableOpacity
                    ...
                    onPress={() => {
                        const reportTitle = `${SECTION_TABS[section].find(t => t.key === activeTab)?.label || 'Report'}`;
                        const summary = `${reportTitle}\n\nProfit: ${currency}${finance.profit.toLocaleString()}\nCash Balance: ${currency}${finance.cashBalance.toLocaleString()}\nRunway: ${finance.runway} days\n\nGenerated on ${new Date().toLocaleDateString()}`;
                        generatePDF({
                            title: reportTitle,
                            ...
                            sections: [
                                {
                                    ...
                                    data: [
                                        { label: 'Profit', value: `${currency}${finance.profit.toLocaleString()}` },
                                        { label: 'Cash Balance', value: `${currency}${finance.cashBalance.toLocaleString()}` },
                                        { label: 'Runway', value: `${finance.runway} days` },
                                    ]
                                }
                            ],
                            summary: [
                                { label: 'Generated By', value: 'Quad360' },
                                { label: 'Date', value: new Date().toLocaleDateString() }
                            ]
                        }).then(filePath => {
                            sharePDF(filePath, reportTitle).catch(() => {
                                Alert.alert('Export Complete', 'Report saved to your device');
                            });
                        });
                    }}
                >
